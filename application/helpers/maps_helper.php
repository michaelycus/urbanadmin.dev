...
EOF;
        ...
        foreach ($requerimentos_situacao as $r_c)
        {
            foreach ($r_c as $r)
            {
                $max = max($r->count_requerimentos, $max);
        ...
        if (!empty($requerimentos_situacao))
        {
            // areas
            $i=-1;
            foreach ($requerimentos_situacao as $r_c)
                    ...
                    foreach ($r_c as $r)
                    {
                        $data .= '"'. $r->codename .'": {
                            ...
                            "attrs": { fill: "'. rgb2hex(array(32 + (($max - $r->count_requerimentos) * 82/$max),
                                                               ...
                                                               135 + (($max - $r->count_requerimentos) * 72/$max))) .'"},
                            "text": { "content":  "'. $r->count_requerimentos .'" , attrs: {fill:"#222"} },
                            "tooltip": { "content": "<span style=\"font-weight:bold;\">'. $r->nome_bairro .'</span><br />Requerimentos: '. $r->count_requerimentos .'"}';
                        $data .= '                             
                        }';
                        $data .= end($r_c)==
                        $r ? '
...
EOF;
        ...
        if ( ! write_file('js/maps/requerimentos_por_situacao.js', $data))
        {
             echo 'Unable to write the file';
        }
        else
...
//             echo 'Arquivo requerimentos_por_tipo.js gerado!<br />';
        ...
        /***** END - Requerimentos por situacao - END *****/
