<div class="container-fluid">
    <div id="heading" class="page-header">
        <h1><i class="icon20 i-paragraph-justify-3"></i> Editar faixa</h1>
    </div>
    ...
    <div class="row">
        <div class="col-lg-6">
            <div class="panel panel-default">
                ...
                echo form_open('faixas/editar_faixa/'.$faixa->id, 'class="bs-docs-example form-horizontal"');

                if (!empty($error))
                    echo '<div class="alert alert-error">'. $error .'</div>';

                echo validation_errors('<div class="alert alert-error">','</div>');
                if ($this->session->userdata('faixa_editada'))
                {
                    echo '<div class="alert alert-success">'. $this->session->userdata('faixa_editada') .'</div>';
                    $this->session->unset_userdata('faixa_editada');
                }
                ...
                {
                    echo        '<option value="'.$bairro->id.'" '.set_select('id_bairro', $bairro->id, $bairro->id==$faixa->id_bairro).'>'.$bairro->nome.'</option>';
                }
                echo        '</select>';
                echo    '</div>';
                echo '</div>';
