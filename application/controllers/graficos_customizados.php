...
class Graficos_Customizados extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('grafico_customizado_model');
    }
    ...
    {
        $this->data['bairros'] = $this->bairro_model->get_bairros();
        $this->data['grafico'] = $this->grafico_customizado_model->get($id);
        $this->data['valores_bairros'] = $this->grafico_customizado_model->get_chart_values($id);
    ...
    {
        $this->data['charts'] = $this->grafico_customizado_model->get_charts_with_requerentes_by_requerente($_SESSION['id_user']);
    ...
    {        
        $id = $this->grafico_customizado_model->get_next_id() - 1;
        $this->data['chart_values'] = $this->grafico_customizado_model->get_chart_values($id);
        $this->data['chart'] = $this->grafico_customizado_model->get($id);    
    ...
    public function excluir_grafico($id)
    {
        ...
        {
            //deleta arquivo js
            $chart = $this->grafico_customizado_model->get($id);
            delete_chart($chart->code);
            
            //deleta banco
            $this->grafico_customizado_model->delete($id);
    ...
    {
        $next_id = $this->grafico_customizado_model->get_next_id();
        ...
        $this->grafico_customizado_model->insert($data);

        $bairros = $this->bairro_model->get_bairros();
...
//                $data_bairro['valor'] = str_replace(',','.',$this->input->post('bairro_'.$bairro->codename)); 
            ...
            $this->grafico_customizado_model->inserir_dados_bairro($data_bairro);
        }
    ...
    public function atualizar_grafico($id)
    {
        ...
        $this->grafico_customizado_model->update($id, $data);

        $bairros = $this->bairro_model->get_bairros();
        
        $this->grafico_customizado_model->deletar_dados_bairro($id);
...
//                $data_bairro['valor'] = str_replace(',','.',$this->input->post('bairro_'.$bairro->codename));
    ...
    {
        $this->data['chart'] = $chart = $this->grafico_customizado_model->get_by_code($code);        
        $this->data['chart_values'] = $this->grafico_customizado_model->get_chart_values($chart->id);        
